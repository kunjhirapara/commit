import { getCurrentUserId } from "@/lib/auth/serverSession";
 ...
 * home to a new path, which every "back home" link and post-sign-in redirect
 * points at.
 *
 * The session is resolved here on the server rather than by the <SignedOut> and
 * <SignedIn> guards. Those are client components: during SSR they cannot know
 * whether anyone is signed in, so both rendered nothing and the entire page
 * arrived empty. The marketing copy only appeared once the auth client had
 * downloaded, booted and settled in the browser — which put mobile LCP at 6.5s against 1.3s
