...
/**
 * Scoped to integrity monitoring on purpose — "we" would be false.
 *
 * The call itself may be recorded, camera and microphone included, so an
 * unqualified "we do not analyse your camera" reads as a promise the product
 * does not keep. It is only true of the integrity signals, and CALL_RECORDING
 * immediately follows it to say what the other half does.
 */
const NOT_RECORDED =
  "Integrity monitoring does not analyse your camera or microphone, capture your screen, record what you type, or look at anything outside this tab.";

/**
 * Consent for recording the call, which is a different thing from integrity
 * monitoring and has to be said separately.
 *
 * Added to the hardcoded disclosure by the recording-consent work on main, and
 * carried here when the copy became per-mode. Without it the paragraph above
 * would be the only mention of the camera, and a candidate would reasonably
 * conclude it is never recorded.
 */
const CALL_RECORDING =
  "Separately, your interviewer may record the call, including your camera and microphone. If they start recording you will see a red “Recording” indicator for as long as it runs. Recordings are available to the hiring team.";
...
export const INTEGRITY_DISCLOSURE: Record<MonitoredMode, DisclosureCopy> = {
  observe: {
    ...
    paragraphs: [RECORDED_OBSERVE, NOT_RECORDED, CALL_RECORDING, AUDIENCE],
    rules: [],
    fullscreen: {
      required: false,
    ...
    acknowledgement:
      "I understand this interview is monitored, and may be recorded, as described above.",
  },
  ...
  deterrent: {
    ...
    acknowledgement:
      "I understand this interview is monitored, may be recorded, and has the rules described above.",
  },
};
